...
import com.eunoiaos.platform.internal.common.UserContentObserver;
import com.eunoiaos.platform.internal.display.TwilightTracker.TwilightListener;
import com.eunoiaos.platform.internal.display.TwilightTracker.TwilightState;
...
import eunoiaos.hardware.HSIC;
import eunoiaos.hardware.ILiveDisplayService;
import eunoiaos.hardware.LiveDisplayConfig;
import eunoiaos.providers.EunoiaSettings;

import static eunoiaos.hardware.LiveDisplayManager.MODE_FIRST;
import static eunoiaos.hardware.LiveDisplayManager.MODE_LAST;
import static eunoiaos.hardware.LiveDisplayManager.MODE_OFF;
 ...
 * and calibration. It interacts with EunoiaHardwareService to relay
 * changes down to the lower layers.
 */
public class LiveDisplayService extends EunoiaSystemService {
    ...
    public String getFeatureDeclaration() {
        return EunoiaContextConstants.Features.LIVEDISPLAY;
    }
    ...
    @Override
    public void onStart() {
        publishBinderService(EunoiaContextConstants.EUNOIA_LIVEDISPLAY_SERVICE, mBinder);
    }
    ...
    public void onBootPhase(int phase) {
        if (phase == PHASE_BOOT_COMPLETED) {
            ...
            int defaultMode = mContext.getResources().getInteger(
                    com.eunoiaos.platform.internal.R.integer.config_defaultLiveDisplayMode);
            ...
            Intent intent = new Intent(eunoiaos.content.Intent.ACTION_INITIALIZE_LIVEDISPLAY);
    ...
    private final IBinder mBinder = new ILiveDisplayService.Stub() {
        ...
        public boolean setMode(int mode) {
            mContext.enforceCallingOrSelfPermission(
                    eunoiaos.platform.Manifest.permission.MANAGE_LIVEDISPLAY, null);
            if (!mConfig.hasModeSupport()) {
                return false;
            }
    ...
    private final class ModeObserver extends UserContentObserver {
        ...
        private final Uri MODE_SETTING =
                EunoiaSettings.System.getUriFor(EunoiaSettings.System.DISPLAY_TEMPERATURE_MODE);
        ...
        int getMode() {
            return getInt(EunoiaSettings.System.DISPLAY_TEMPERATURE_MODE,
                    mConfig.getDefaultMode());
        }

        boolean setMode(int mode) {
            if (mConfig.hasFeature(mode) && mode >= MODE_FIRST && mode <= MODE_LAST) {
                putInt(EunoiaSettings.System.DISPLAY_TEMPERATURE_MODE, mode);
    ...
    private void nudge() {
        ...
        if (counter == 0) {
            ...
            final Intent intent = new Intent(EunoiaSettings.ACTION_LIVEDISPLAY_SETTINGS);
            PendingIntent result = PendingIntent.getActivity(mContext, 0, intent,
                    PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_UPDATE_CURRENT);
            Notification.Builder builder = new Notification.Builder(mContext)
                    .setContentTitle(mContext.getResources().getString(
                            com.eunoiaos.platform.internal.R.string.live_display_title))
                    .setContentText(mContext.getResources().getString(
                            com.eunoiaos.platform.internal.R.string.live_display_hint))
                    .setSmallIcon(com.eunoiaos.platform.internal.R.drawable.ic_livedisplay_notif)

...
import static eunoiaos.hardware.LiveDisplayManager.MODE_OUTDOOR;
...
public class OutdoorModeController extends LiveDisplayFeature {
    ...
    public OutdoorModeController(Context context, Handler handler) {
        ...
        mUseOutdoorMode = mHardware.isSupported(EunoiaHardwareManager.FEATURE_SUNLIGHT_ENHANCEMENT);

        mDefaultOutdoorLux = mContext.getResources().getInteger(
                com.eunoiaos.platform.internal.R.integer.config_outdoorAmbientLux);
        mOutdoorLuxHysteresis = mContext.getResources().getInteger(
                com.eunoiaos.platform.internal.R.integer.config_outdoorAmbientLuxHysteresis);
        mDefaultAutoOutdoorMode = mContext.getResources().getBoolean(
                com.eunoiaos.platform.internal.R.bool.config_defaultAutoOutdoorMode);
    }
    ...
    public void onStart() {
        ...
        registerSettings(
                EunoiaSettings.System.getUriFor(EunoiaSettings.System.DISPLAY_AUTO_OUTDOOR_MODE));
    }
    ...
    protected synchronized void onScreenStateChanged() {
        ...
        if (!isScreenOn() && getMode() != MODE_OUTDOOR) {
            mIsOutdoor = false;
            mHardware.set(EunoiaHardwareManager.FEATURE_SUNLIGHT_ENHANCEMENT, false);
        }
    }
    ...
    public void dump(PrintWriter pw) {
        ...
        pw.println("    hardware state=" +
                mHardware.get(EunoiaHardwareManager.FEATURE_SUNLIGHT_ENHANCEMENT));
    ...
    private synchronized void updateOutdoorMode() {
        ...
        if (isScreenOn()) {
            ...
            mHardware.set(EunoiaHardwareManager.FEATURE_SUNLIGHT_ENHANCEMENT, enabled);
        }
    }
    ...
    boolean setAutomaticOutdoorModeEnabled(boolean enabled) {
        ...
        putBoolean(EunoiaSettings.System.DISPLAY_AUTO_OUTDOOR_MODE, enabled);
        return true;
    }

    boolean isAutomaticOutdoorModeEnabled() {
        return mUseOutdoorMode && (mNightDisplayAvailable ||
                getBoolean(EunoiaSettings.System.DISPLAY_AUTO_OUTDOOR_MODE,

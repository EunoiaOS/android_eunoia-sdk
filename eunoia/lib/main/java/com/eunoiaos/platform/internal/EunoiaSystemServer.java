...
import com.eunoiaos.platform.internal.common.EunoiaSystemServiceHelper;

/**
 * Base Eunoia System Server which handles the starting and states of various Eunoia
 ...
 */
public class EunoiaSystemServer {
    private static final String TAG = "EunoiaSystemServer";
    private Context mSystemContext;
    private EunoiaSystemServiceHelper mSystemServiceHelper;

    private static final String ENCRYPTING_STATE = "trigger_restart_min_framework";
    private static final String ENCRYPTED_STATE = "1";

    public EunoiaSystemServer(Context systemContext) {
        mSystemContext = systemContext;
        mSystemServiceHelper = new EunoiaSystemServiceHelper(mSystemContext);
    }
    ...
    private void run() {
        ...
        try {
            ...
            Slog.e("System", "******************************************");
            Slog.e("System", "************ Failure starting eunoiaosystem services", ex);
            throw ex;
        }
    }
    ...
    private void startServices() {
        ...
        String[] externalServices = context.getResources().getStringArray(
                com.eunoiaos.platform.internal.R.array.config_externalEunoiaServices);

        for (String service : externalServices) {
            try {
                Slog.i(TAG, "Attempting to start service " + service);
                EunoiaSystemService eunoiaSystemService =  mSystemServiceHelper.getServiceFor(service);
                if (context.getPackageManager().hasSystemFeature(
                        eunoiaSystemService.getFeatureDeclaration())) {
                    if (coreAppsOnly() && !eunoiaSystemService.isCoreService()) {
                        Slog.d(TAG, "Not starting " + service +
                                " - only parsing core apps");
                    } else {
                        Slog.i(TAG, "Starting service " + service);
                        ssm.startService(eunoiaSystemService.getClass());
                    }
                } else {

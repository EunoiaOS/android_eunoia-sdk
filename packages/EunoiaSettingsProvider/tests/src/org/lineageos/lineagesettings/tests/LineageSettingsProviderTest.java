...
import java.util.Map;
 ...
 public class EunoiaSettingsProviderTest extends AndroidTestCase {
     private static final String TAG = "EunoiaSettingsProviderTest";
     ...
     @MediumTest
     public void testMigrateEunoiaSettingsForOtherUser() {
     ...
     private void testMigrateSettingsForUser(int userId) {
         ...
         Settings.System.putStringForUser(mContentResolver,
                 EunoiaSettingsProvider.LegacyEunoiaSettings.STATUS_BAR_QUICK_QS_PULLDOWN,
                 expectedPullDownValue, userId);

         final int expectedKeyboardBrightness = 4;
         Settings.Secure.putIntForUser(mContentResolver,
                 EunoiaSettingsProvider.LegacyEunoiaSettings.KEYBOARD_BRIGHTNESS,
                 expectedKeyboardBrightness, userId);

         Bundle arg = new Bundle();
         arg.putInt(EunoiaSettings.CALL_METHOD_USER_KEY, userId);
         IContentProvider contentProvider = mContentResolver.acquireProvider(
                 EunoiaSettings.AUTHORITY);

         try{
             // Trigger migrate settings for guest
             contentProvider.call(mContentResolver.getPackageName(),
                     EunoiaSettings.CALL_METHOD_MIGRATE_SETTINGS_FOR_USER, null, arg);
         } catch (RemoteException ex) {
             fail("Failed to trigger settings migration due to RemoteException");
         }

         // Check values
         final String actualPullDownValue = EunoiaSettings.System.getStringForUser(mContentResolver,
                 EunoiaSettings.System.QS_QUICK_PULLDOWN, userId);
         assertEquals(expectedPullDownValue, actualPullDownValue);

         final int actualKeyboardBrightness = EunoiaSettings.Secure.getIntForUser(mContentResolver,
                 EunoiaSettings.Secure.KEYBOARD_BRIGHTNESS, -1, userId);
     ...
     public void testBulkInsertSuccess() {
         ...
         testBulkInsertForUri(EunoiaSettings.System.CONTENT_URI, contentValues, keyValues);
         testBulkInsertForUri(EunoiaSettings.Secure.CONTENT_URI, contentValues, keyValues);
         testBulkInsertForUri(EunoiaSettings.Global.CONTENT_URI, contentValues, keyValues);
     }
     ...
     public void testInsertUpdateDeleteSuccess() {
         //testInsertUpdateDeleteForUri(EunoiaSettings.System.CONTENT_URI);
         testInsertUpdateDeleteForUri(EunoiaSettings.Secure.CONTENT_URI);
         testInsertUpdateDeleteForUri(EunoiaSettings.Global.CONTENT_URI);
     }

...
public class PreBootReceiver extends BroadcastReceiver{
    private static final String TAG = "EunoiaSettingsReceiver";
    private static final boolean LOCAL_LOGV = false;

    @Override
    public void onReceive(Context context, Intent intent) {
        if (LOCAL_LOGV) {
            Log.d(TAG, "Received pre boot intent. Attempting to migrate Eunoia settings.");
        }
        ...
        try {
            contentProvider.call(contentResolver.getAttributionSource(),
                    EunoiaSettings.AUTHORITY,
                    EunoiaSettings.CALL_METHOD_MIGRATE_SETTINGS, null, null);

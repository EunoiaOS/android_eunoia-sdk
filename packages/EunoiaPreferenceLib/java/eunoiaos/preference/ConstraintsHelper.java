...
import static eunoiaos.preference.R.styleable.eunoia_SelfRemovingPreference_minSummaryLines;
import static eunoiaos.preference.R.styleable.eunoia_SelfRemovingPreference_replacesKey;
...
public class ConstraintsHelper {
    ...
    public ConstraintsHelper(Context context, AttributeSet attrs, Preference pref) {
        ...
        TypedArray a = context.getResources().obtainAttributes(attrs,
                R.styleable.eunoia_SelfRemovingPreference);
        mSummaryMinLines = a.getInteger(eunoia_SelfRemovingPreference_minSummaryLines, -1);
        String replacesKey = a.getString(eunoia_SelfRemovingPreference_replacesKey);
        if (replacesKey != null) {
    ...
    private boolean checkConstraints() {
        ...
        try {
            ...
            boolean rOwner = a.getBoolean(R.styleable.eunoia_SelfRemovingPreference_requiresOwner, false);
            if (rOwner && UserHandle.myUserId() != UserHandle.USER_SYSTEM) {
                return false;
            }

            // Check if a specific package is installed
            String rPackage = a.getString(R.styleable.eunoia_SelfRemovingPreference_requiresPackage);
            if (rPackage != null) {
            ...
            String rAction = a.getString(R.styleable.eunoia_SelfRemovingPreference_requiresAction);
            if (rAction != null) {
            ...
            String rFeature = a.getString(R.styleable.eunoia_SelfRemovingPreference_requiresFeature);
            if (rFeature != null) {
                boolean negated = isNegated(rFeature);
                if (negated) {
                    rFeature = rFeature.substring(1);
                }
                boolean available = rFeature.startsWith("eunoiahardware:") ?
                        EunoiaHardwareManager.getInstance(mContext).isSupported(
                                rFeature.substring("eunoiahardware:".length())) :
            ...
            String rProperty = a.getString(R.styleable.eunoia_SelfRemovingPreference_requiresProperty);
            if (rProperty != null) {
            ...
            TypedValue tv = a.peekValue(R.styleable.eunoia_SelfRemovingPreference_requiresConfig);
            if (tv != null && tv.resourceId != 0) {
                if (tv.type == TypedValue.TYPE_STRING &&
                    ...
                    int mask = a.getInt(
                            R.styleable.eunoia_SelfRemovingPreference_requiresConfigMask, -1);
            ...
            String rService = a.getString(
                    R.styleable.eunoia_SelfRemovingPreference_requiresService);
            if (rService != null) {

...
/**
 * Manages access to EunoiaOS hardware extensions
 *
 *  <p>
 *  This manager requires the HARDWARE_ABSTRACTION_ACCESS permission.
 *  <p>
 *  To get the instance of this class, utilize EunoiaHardwareManager#getInstance(Context context)
 */
public final class EunoiaHardwareManager {
    private static final String TAG = "EunoiaHardwareManager";
    ...
    private static IEunoiaHardwareService sService;
    private static EunoiaHardwareManager sEunoiaHardwareManagerInstance;
     ...
     */
    private EunoiaHardwareManager(Context context) {
        ...
        if (context.getPackageManager().hasSystemFeature(
                EunoiaContextConstants.Features.HARDWARE_ABSTRACTION) && !checkService()) {
            Log.wtf(TAG, "Unable to get EunoiaHardwareService. The service either" +
                    " crashed, was not started, or the interface has been called to early in" +
                    " SystemServer init");
        }

        final String[] mappings = mContext.getResources().getStringArray(
                com.eunoiaos.platform.internal.R.array.config_displayModeMappings);
        ...
        mFilterDisplayModes = mContext.getResources().getBoolean(
                com.eunoiaos.platform.internal.R.bool.config_filterDisplayModes);
    }

    /**
     * Get or create an instance of the {@link eunoiaos.hardware.EunoiaHardwareManager}
     * @param context
     * @return {@link EunoiaHardwareManager}
     */
    public static EunoiaHardwareManager getInstance(Context context) {
        if (sEunoiaHardwareManagerInstance == null) {
            sEunoiaHardwareManagerInstance = new EunoiaHardwareManager(context);
        }
        return sEunoiaHardwareManagerInstance;
    }

    /** @hide */
    public static IEunoiaHardwareService getService() {
        if (sService != null) {
            return sService;
        }
        IBinder b = ServiceManager.getService(EunoiaContextConstants.EUNOIA_HARDWARE_SERVICE);
        if (b != null) {
            sService = IEunoiaHardwareService.Stub.asInterface(b);
            return sService;
        }
        return null;
    }
    ...
    /**
     * Determine if a Eunoia Hardware feature is supported on this device
     *
     * @param feature The Eunoia Hardware feature to query
     *
     ...
     * Only used for features which have simple enable/disable controls.
     *
     * @param feature the Eunoia Hardware feature to set
    ...
    private boolean checkService() {
        if (sService == null) {
            Log.w(TAG, "not connected to EunoiaHardwareManagerService");
            return false;
        }
        return true;
    }

...
public class BuildTest extends AndroidTestCase {
    ...
    public void testFetchSdkApiLevelExists() {
        assertNotNull(Build.EUNOIA_VERSION.SDK_INT);
    }

    @SmallTest
    public void testSdkApiLevelCurrent() {
        assertEquals(Concierge.PARCELABLE_VERSION, Build.EUNOIA_VERSION.SDK_INT);
    }

    @SmallTest
    public void testSdkApiLevelCanMatch() {
        String apiName = Build.getNameForSDKInt(Build.EUNOIA_VERSION.SDK_INT);
    ...
    public void testSdkApiLevelSkippedIfGreaterThanAllowed() {
        int i = 0;
        if (Build.EUNOIA_VERSION.SDK_INT > Concierge.PARCELABLE_VERSION + 1) {
            i++;
        }
        assertEquals(0, i);
    }
